...
def generate_text(model, char_to_id, id_to_char, hidden_dim, prompt, max_length=250):
    model.eval()
...
class ShahnamehGUI:
    ...
    def on_generate(self):
        ...
        threading.Thread(target=self._generate_thread, args=(prompt, max_len), daemon=True).start()

    def _generate_thread(self, prompt, max_len):
        try:
            out = generate_text(self.model, self.char_to_id, self.id_to_char, self.hidden_dim, prompt, max_length=max_len)
            full = prompt + out
            
            # Keep newlines for proper line breaks
            cleaned_out = out
            # Only remove carriage returns and tabs, keep newlines
            cleaned_out = cleaned_out.replace('\r', '').replace('\t', ' ')
